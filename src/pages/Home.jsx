...
const MangaCard = ({ a, onClick, badge }) => (
  <div onClick={onClick} className="min-w-[105px] w-[105px] group cursor-pointer snap-start active:scale-95 flex flex-col gap-2 transition-transform">
    <div className="relative aspect-[3/4.5] overflow-hidden bg-[#16161a] rounded-sm shadow-xl">
      ...
      {badge && <div className="absolute top-1 left-1 bg-black/70 text-[#F6CF80] text-[8px] font-black px-1.5 py-0.5 rounded-sm">{badge}</div>}
...
const Home = () => {
  ...
  // Hero carousel (gunakan carousel; fallback ke popular)
              ...
              <div key={i} className="min-w-full h-full relative">
                ...
                <div className="absolute bottom-6 left-6 md:bottom-12 md:left-12 flex items-end gap-4 z-10 w-[calc(100%-48px)] max-w-7xl pr-8">
                  ...
                  <div className="flex flex-col text-left mb-1 gap-1 flex-1 min-w-0">
                    ...
                    {item.slug && (
                      <button onClick={() => navigate(`/manga/${item.slug}`)} className="mt-2 h-8 px-5 bg-[#F6CF80] hover:bg-[#ebd59b] text-black rounded font-black tracking-wider text-[10px] flex items-center gap-1.5 w-fit transition-colors">
